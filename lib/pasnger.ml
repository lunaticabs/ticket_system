...
let add_passenger (passenger_list : passenger list) (p : passenger) :
    passenger list =
  p :: passenger_list

(* Delete a passenger when condition is met *)
let delete_passenger (passenger_list : passenger list)
    (cond : passenger -> bool) : passenger list =
...
let find_passenger (passenger_list : passenger list) (cond : passenger -> bool)
    : passenger option =
  try Some (List.find cond passenger_list) with Not_found -> None

(* Modify a passenger in the list if condition is met *)
let modify_passenger (passenger_list : passenger list)
    (cond : passenger -> bool) (update : passenger -> passenger) :
    passenger list =
