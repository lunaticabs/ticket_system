...
type ticket_node =
  | Empty
  | Node of ticket_info * ticket_node ref
...
let find_ticket head condition =
  let rec aux node =
    ...
    | Empty -> None
    | Node (ticket, next) ->
        if condition ticket then Some ticket else aux next
  in
  aux head
...
let available_tickets head travel_date train_id start_station end_station total_seats =
  let rec aux node count =
    ...
    | Node (ticket, next) ->
        if ticket.travel_date = travel_date &&
           ticket.start_station = start_station &&
           ticket.end_station = end_station then
          aux next (count + 1)
        else
          aux next count
  in
...
let output_all_tickets head =
  let rec aux node =
    ...
    | Empty -> ()
    | Node (ticket, next) ->
        Printf.printf "订单号: %s, 姓名: %s, 日期: %s, 起点: %s, 终点: %s, 证件号: %s, 开车时间: %s, 票价: %.2f, 座位: %d, 订票时间: %s, 坐席: %s\n"
